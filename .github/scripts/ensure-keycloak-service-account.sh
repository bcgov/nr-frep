...
# realm for the FREP backend's callouts to the nr-user-lookup-api, with the
...
# Optional:
#   FREP_CLIENT_ID        clientId of the service account to ensure
#                         (default: nr-frep-backend)
#
...
FREP_CLIENT_ID="${FREP_CLIENT_ID:-nr-frep-backend}"
...
echo "Ensuring service-account client: ${FREP_CLIENT_ID}"
...
if [ -n "${uuid}" ]; then
  ...
  body="$(jq -n --arg id "${FREP_CLIENT_ID}" '{
    clientId: $id,
    name: "NR FREP backend (user-lookup callouts)",
    description: "Service account for the FREP backend → nr-user-lookup-api. Managed by nr-frep CI.",
...
echo "Done. '${FREP_CLIENT_ID}' ready with ${#SCOPES[@]} scope(s); client_id/client_secret exposed as step outputs."
