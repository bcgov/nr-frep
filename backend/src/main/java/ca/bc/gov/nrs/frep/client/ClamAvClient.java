 ...
 *
 * <p>Copied from nr-fspts {@code ClamAvClient}.
 */
@Component
public class ClamAvClient {
  ...
  @Autowired
  ClamAvClient(
      @Value("${frep.clamav.host:}") String host,
      @Value("${frep.clamav.port:3310}") int port,
      @Value("${frep.clamav.connect-timeout:5s}") Duration connectTimeout,
      @Value("${frep.clamav.read-timeout:30s}") Duration readTimeout,
      @Value("${frep.clamav.chunk-size:8192}") int chunkSize) {
    this(host, port,
