...
test.describe('CHR Features — Age is single-select', () => {
  test('selecting one age disables the other three', async ({ page }) => {
    ...
    if ((await firstChr.count()) === 0) {
      test.skip(
        true,
        'No Active CHR checklist available on this environment to exercise the editor.',
      );
    }
    await firstChr.click();

    // 2. Open the Features tab. If the checklist isn't editable for this user, there's nothing to test.
    await page.getByRole('tab', { name: 'Features' }).click();
    const addFeature = page.getByRole('button', { name: 'Add feature' });
    if (!(await addFeature.isVisible().catch(() => false))) {
      test.skip(true, 'Checklist is read-only for this user — the Age editor is not reachable.');
    }
