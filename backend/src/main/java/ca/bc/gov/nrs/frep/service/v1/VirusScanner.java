...
import ca.bc.gov.nrs.frep.client.ClamAvClient;
import ca.bc.gov.nrs.frep.client.ScanResult;
import ca.bc.gov.nrs.frep.exception.VirusDetectedException;
 ...
 *
 * <ul>
 *   <li>{@code frep.clamav.enabled} (default {@code false}) — when off,
...
public class VirusScanner {
  ...
  public VirusScanner(
      ClamAvClient client,
      @Value("${frep.clamav.enabled:false}") boolean enabled,
      @Value("${frep.clamav.fail-open:false}") boolean failOpen) {
    this.client = client;
  ...
  void logScannerHealthAtStartup() {
    if (!enabled) {
      LOG.info("Virus scanning DISABLED (frep.clamav.enabled=false) — uploads are not scanned.");
      return;
    }
