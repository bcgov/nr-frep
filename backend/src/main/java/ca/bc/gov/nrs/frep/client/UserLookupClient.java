package ca.bc.gov.nrs.frep.client;
 ...
 * resolves IDIR identities. Replaces the FAM identity-lookup integration FREP
 * used previously — BC Gov SSO (Keycloak) is still where <i>authentication</i> and
 * the FREP roles come from, just not the directory.
 *
 * <p>Base path {@code /api/v1/user-lookup}. Every call carries a Keycloak
 * {@code client_credentials} bearer token minted from FREP's own service
 * account (see {@link ClientCredentialsTokenSource}); the caller's own JWT
 ...
 *       {@code .client-secret} — FREP's confidential service-account
 *       credentials</li>
